...
(() => {
  'use strict';
  function __bustrFatal(e) {
    try {
      var d = document.createElement("div");
      d.style.cssText = "position:fixed;left:3px;right:3px;top:3px;z-index:2147483647;"
        + "background:#300;color:#f99;font:11px/1.4 monospace;padding:8px;border:1px solid #f99;"
        + "border-radius:5px;max-height:70vh;overflow:auto;white-space:pre-wrap";
      var msg = (e && e.stack) ? e.stack : String(e);
      d.textContent = "BUSTR FATAL - threw at module level (tap to dismiss)" + String.fromCharCode(10)
  ...
  //   - PDA's ###PDA-APIKEY### substitution can land the key WRAPPED IN QUOTES, so
...
@media screen and (max-width:386px) {
  ...
  (async function () {
    try {
      ...
      // Always wire up the jail-row column - it renders BOTH hardness and success %,
      // and visibility of each is handled independently via CSS body classes
      // (applyJailVisibility). Gating this behind showHardnessScore was a bug: it
      // meant success % never initialized at all if the hardness number was hidden.
      hardnessScoreController();
      successfulBustUpdateController();
