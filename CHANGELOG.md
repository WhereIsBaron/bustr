...
## v2.15.1
- [CHANGED] On PDA, when the injected key is working the settings panel now shows the same **"API key is saved."** status as a saved desktop key, instead of a longer PDA-specific line. The opt-in "Use your own key instead" override link is unchanged. Wording only.
