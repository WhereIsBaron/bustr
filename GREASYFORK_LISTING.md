...
- Optional cloud sync (opt-in, off by default): you can choose to back up your bust history so it follows you across devices. It asks for explicit consent the first time you enable it, and uploads your bust history along with the context behind your predictions - your bust perks, level, calibration, BUSTR settings and script version - all so the success model can be improved. Never your API key, name, or faction. It can be turned off and its cloud copy deleted at any time from the settings panel. Leave it off and BUSTR is fully local, exactly as before. When it is on it is deliberately lightweight: a whole busting session is saved in a single upload and it only checks for changes from your other devices about once an hour, so it adds no noticeable overhead.
